...
def Elementum(): #97
	...
	if d == 0:
		...
		j = json.loads(v)
		listitem = xbmcgui.ListItem(path="plugin://plugin.video.elementum/library/movie/play/"+j['imdbnumber']+"?doresume=true")
		xbmcplugin.setResolvedUrl(int(sys.argv[1]), True, listitem)

def PlayUrl2(name, url, iconimage=None, info='', sub='', metah=''):
	RS = ReadSeek(metah['imdbnumber'])
	...
	if metah:
		if background and episode:
			...
			if 'EpisodeTitle' in eInfo2:
				...
				liz=xbmcgui.ListItem(DL+"[COLOR white]"+background+"x"+EPI(episode)+". Episode "+EPI(episode)+"[/COLOR]")
